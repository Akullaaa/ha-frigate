...
case "$MODE" in
  ...
  base|b[0-9]*) export OV_W=960 OV_H=1080 OV_SCALE=1.6 OV_TWO_LISTS=1; PRE="scale=960:1080,"; BV=3M; MAXR=6M; SCS=0.3
                OUTFPS=13; [ "$MODE" != base ] && OUTFPS=${MODE#b};;
...
export OV_DIR=/tmp/xm530_overlay_$MODE
export OV_SCOPE_SCALE=$(awk -v s="$OV_SCALE" -v k="${SCS:-0.75}" 'BEGIN{printf "%.3f", s*k}')
export GO2RTC_AUTH="alena:Rm9g8fsLobAS6I2jfoEY"
SRC="rtsp://alena:Rm9g8fsLobAS6I2jfoEY@127.0.0.1:8554/xm530_cam"   # чистый поток камеры (до 2026-09-15 — xm530)
FONT=/usr/share/fonts/truetype/dejavu/DejaVuSansMono-Bold.ttf
mkdir -p /config/logs; LOG=/config/logs/xm530_overlay_$MODE.log   # 2026-09-15: в /config (виден из аддона Studio Code Server как /addon_configs/ccab4aaf_frigate-fa/logs), был /tmp контейнера
...
python3 /config/xm530_overlay_collect.py $$ 2>>"$LOG" &
( while kill -0 $$ 2>/dev/null; do python3 /config/xm530_overlay_render.py $$ --noclock 2>>"$LOG"; sleep 1; done ) &
