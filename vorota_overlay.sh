...
# 2026-09-15 (вечер), по решению пользователя: режим base — 960x1080 @ 25 к/с (камера даёт 20, fps= дублирует), 3 Мбит/с — это САМ
...
MODE="${2:-full}"
case "$MODE" in
  # base — базовый поток vorota (detect/record/live по умолчанию): 25 к/с (было 34, по просьбе пользователя). b<N> — те же настройки (960x1080, шрифт 1.3,
  # два списка) с другой частотой, отдельные потоки go2rtc vorota_b<N> только для просмотра, в запись не идут
  # (пробовали 25/34/43/60/70/90: CPU ffmpeg 85 % до 34, 93 % на 60, 104 % на 90; на 90 полосы при просмотре с Debian).
  base|b[0-9]*) export OV_W=960 OV_H=1080 OV_SCALE=1.3 OV_TWO_LISTS=1; PRE="scale=960:1080,"; BV=3M; MAXR=6M; SCS=0.45
                OUTFPS=25; [ "$MODE" != base ] && OUTFPS=${MODE#b};;
