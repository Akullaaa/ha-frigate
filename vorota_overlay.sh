...
# 2026-09-15 (вечер), по решению пользователя: режим base — 960x1080 @ 70 к/с (камера даёт 20, fps= дублирует), 3 Мбит/с — это САМ
...
MODE="${2:-full}"
case "$MODE" in
  base)   export OV_W=960 OV_H=1080 OV_SCALE=1.3 OV_TWO_LISTS=1; OUTFPS=70; PRE="scale=960:1080,"; BV=3M; MAXR=6M; SCS=0.45;;   # 70 к/с (кольцо — 70 точек; на 90 у пользователя горизонтальные полосы при просмотре с Debian; 25/34 — 85 % CPU у ffmpeg, 60 — 93 %, 90 — 104 %); шрифт 1.3 (списки впритык по высоте, ради него убраны 6 строк в сборщике), шрифт 1.2 и два списка деревом (левый: всё, правый: HA) по просьбе пользователя; скопы мельче
...
mkdir -p /config/logs; LOG=/config/logs/vorota_overlay_$MODE.log   # 2026-09-15: в /config (виден из аддона Studio Code Server как /addon_configs/ccab4aaf_frigate-fa/logs), был /tmp контейнера
...
CLOCK="$DT:fontsize=$CFS:x=$CX:y=$CY:text='%{localtime\:%F  %T.%4N} (%{n})'"   # 2026-09-15: счётчик кадров в скобках после долей секунды (по просьбе пользователя), отдельной подписи «кадр N» больше нет
