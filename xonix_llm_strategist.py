...
DECIDE_INTERVAL = 30.0  # секунд между обращениями к LLM — не на каждый тик.
# Было 20с — увеличено по прямому запросу пользователя (2026-08-21) ради
# экономии: reasoning-токены (не входные) — основной вклад в цену запроса,
# а не выбор модели — более дешёвой модели на аккаунте не нашлось, k3 с
# think_effort=low дал лишь ~5% экономии (в пределах шума, см. проверку
# вживую в project_xonix_game.md), поэтому единственный реальный рычаг —
# звонить реже. 30с вместо 20с — на треть меньше вызовов и, соответственно,
# на треть меньше расход, без смены модели или поведения бота.
